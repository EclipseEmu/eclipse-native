...
func withCheckedBlockingContinuation<T>(
    ...
    body: @Sendable @escaping (CheckedContinuation<T, Never>) -> Void
) async -> T {
...
func withCheckedBlockingThrowingContinuation<T>(
    queue: DispatchQueue = .global(),
    body: @Sendable @escaping (CheckedContinuation<T, any Error>) -> Void
) async throws -> T {
...
func withUnsafeBlockingContinuation<T>(
    queue: DispatchQueue = .global(),
    body: @Sendable @escaping (UnsafeContinuation<T, Never>) -> Void
) async -> T {
...
func withUnsafeBlockingThrowingContinuation<T>(
    queue: DispatchQueue = .global(),
    body: @Sendable @escaping (UnsafeContinuation<T, any Error>) -> Void
) async throws -> T {
